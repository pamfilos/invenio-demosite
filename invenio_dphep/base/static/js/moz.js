...
//                 B = F.height();
//             F.text("a");
//             var A = parseFloat(F.css("lineHeight"), 10),
//                 z = F.height(),
//                 y = A > z ? A - z : 0,
//                 x = y * (a.row - 1) + z * a.row;
//             if (x >= B) {
//                 return F.text(E), a.callback.call(this), void 0
//             }
//             var w = 1,
//                 v = 0,
//                 u = E.length;
//             if ("tail" === a.position) {
//                 for (; u > w;) {
//                     v = Math.ceil((w + u) / 2), F.text(E.slice(0, v) + a["char"]), F.height() <= x ? w = v : u = v - 1
//                 }
//                 E = E.slice(0, w), a.onlyFullWords && (E = E.replace(/[\u00AD\w\uac00-\ud7af]+$/, "")), E += a["char"]
//             } else {
//                 if ("middle" === a.position) {
//                     for (var t = 0; u > w;) {
//                         v = Math.ceil((w + u) / 2), t = Math.max(C - v, 0), F.text(D.slice(0, Math.floor((C - t) / 2)) + a["char"] + D.slice(Math.floor((C + t) / 2), C)), F.height() <= x ? w = v : u = v - 1
//                     }
//                     t = Math.max(C - w, 0);
//                     var s = D.slice(0, Math.floor((C - t) / 2)),
//                         r = D.slice(Math.floor((C + t) / 2), C);
//                     a.onlyFullWords && (s = s.replace(/[\u00AD\w\uac00-\ud7af]+$/, "")), E = s + a["char"] + r
//                 }
//             }
//             F.text(E), a.callback.call(this)
//         }), this
//     }
// }(jQuery);
...
(function (f) {
    ...
    function k() {
        ...
        f(".extra-news a").each(function () {
            var l = f(this).attr("title");
            if (!l) {
                l = f.trim(f(this).text());
                f(this).attr("title", l)
            }
            f(this).text(l).ellipsis({
                row: 3
            })
        })
    }
